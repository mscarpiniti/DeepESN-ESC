...
#The SimpleDeepESNStates can be used to obtain the states of the Deep ESN for 
#a future use (e.g., compare several classifiers).
#Added by M. Scarpiniti, Department of Information Engineering, Electronics 
#and Telecommunications (DIET) -  Sapienza University of Rome

class SimpleDeepESNStates(keras.Model):
    def __init__(self, units = 100, layers = 5, 
                 concat = True,
                 spectral_radius = 0.99, leaky = 1,
                 input_scaling = 1, inter_scaling = 1,
                 connectivity_recurrent = 10, 
        ...
        #possibly arbitrarily long input time-series
        self.masking = tf.keras.layers.Masking()
        self.hidden = SimpleDeepReservoirLayer(units = units, 
                                         layers = layers,
                                         concat = concat,
                                         spectral_radius = spectral_radius, 
                                         leaky = leaky,
